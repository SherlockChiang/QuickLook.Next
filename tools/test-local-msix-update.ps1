...
$requiredPatterns = @(
    ...
    @('checked-invocation\.ps1[\s\S]*Invoke-CheckedScript[\s\S]{0,300}pack-msix\.ps1', "Local updates must fail closed on the packaging script."),
    @('pack-msix\.ps1[\s\S]{0,300}Version\s*=\s*\$numericVersion[\s\S]{0,100}SkipBuild\s*=\s*\$true', "Local updates must package the tested build with an explicit four-part version."),
