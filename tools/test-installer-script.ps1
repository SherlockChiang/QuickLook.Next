...
$installedPackageCommand = Assert-OneCommand -Name "Get-AppxPackage"
...
if (@($preflightOffsets | Where-Object {
    ...
    throw "Installer file, signature, identity, and trust preflight must run before elevation."
}
if ($signatureCommands[1].Extent.StartOffset -le $elevationCommands[0].Extent.StartOffset -or
    ...
    throw "Installer trust verification, installation, and postcondition checks are out of order."
}
...
$requiredPatterns = @(
    ...
    @('Get-AppxPackage\s+-Name\s+\$expectedPackageName[\s\S]{0,500}\.Name\s+-ne\s+\$packageIdentity\.Name[\s\S]{0,200}\.Publisher\s+-ne\s+\$packageIdentity\.Publisher[\s\S]{0,200}Version\.ToString\(\)\s+-ne\s+\$packageIdentity\.Version', "The installed current-user package must match the manifest name, publisher, and version."),
