...
Require-Pattern $parserHostIntegration 'TransferredHandleReleaseTimeout\s*=\s*TimeSpan\.FromSeconds\(20\)[\s\S]*Duplicate_handle_request_ID_releases_every_transferred_handle[\s\S]*CancellationTokenSource\(TransferredHandleReleaseTimeout\)[\s\S]*TryOverwriteFile\(firstPath[\s\S]*releaseTimeout\.Token[\s\S]*Duplicate_SQLite_handle_request_ID_releases_both_handle_bundles[\s\S]*CancellationTokenSource\(TransferredHandleReleaseTimeout\)[\s\S]*paths\.All\(path\s*=>\s*TryOverwriteFile[\s\S]*releaseTimeout\.Token' `
    "ParserHost duplicate HANDLE tests must keep an independent bounded native-cancellation release budget."
