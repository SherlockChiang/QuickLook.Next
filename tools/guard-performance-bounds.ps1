...
$nativeMailCfb = Join-Path $Root "native/quicklook_next_native/src/preview/mail/cfb.rs"
$nativeMailTests = Join-Path $Root "native/quicklook_next_native/src/preview/mail/tests.rs"
$nativeMailCfbTests = Join-Path $Root "native/quicklook_next_native/src/preview/mail/cfb/tests.rs"
...
Require-Pattern $nativeMailPreview 'MAX_MAIL_HANDLE_INPUT_BYTES:\s*u64\s*=\s*256\s*\*\s*1024\s*\*\s*1024[\s\S]*render_mail_reader<R:\s*Read\s*\+\s*Seek>[\s\S]*source_len\s*>\s*MAX_MAIL_HANDLE_INPUT_BYTES[\s\S]*prepare_seekable_reader\(&mut reader, source_len, cancel_cb\)[\s\S]*read_reader_prefix_cancelable\(&mut reader, cfb::CFB_SIGNATURE\.len\(\), cancel_cb\)[\s\S]*signature\.starts_with\(&cfb::CFB_SIGNATURE\)[\s\S]*append_msg_compound_summary\(&mut text, &mut reader, source_len, cancel_cb\)[\s\S]*SeekFrom::Start\(0\)[\s\S]*read_reader_prefix_cancelable\(&mut reader, MAX_MAIL_HEADER_BYTES, cancel_cb\)[\s\S]*headers\.len\(\)\s*>=\s*MAX_MAIL_HEADERS[\s\S]*\.take\(MAX_MAIL_HEADER_PARAMETERS\)[\s\S]*encoded_words\s*>=\s*MAX_MAIL_ENCODED_WORDS[\s\S]*decode_base64_into\(value, max_bytes[\s\S]*filenames\.len\(\)\s*<\s*MAX_MAIL_ATTACHMENT_NAMES[\s\S]*depth\s*>\s*MAX_MAIL_MIME_DEPTH[\s\S]*summaries\.len\(\)\s*>=\s*MAX_MAIL_MIME_PARTS[\s\S]*trimmed\.len\(\)\s*>\s*MAX_MAIL_DECODED_BODY_BYTES' `
    "Mail HANDLE routing and MIME parsing must retain the source cap, cancellable seek pipeline, 256 KiB EML prefix, and declared collection/decode budgets."
Require-Pattern $nativeMailCfb 'MAX_CFB_FAT_SECTORS:\s*usize\s*=\s*16[\s\S]*MAX_CFB_DIFAT_SECTORS:\s*usize\s*=\s*8[\s\S]*MAX_CFB_DIRECTORY_SECTORS:\s*usize\s*=\s*16[\s\S]*MAX_CFB_DIRECTORY_ENTRIES:\s*usize\s*=\s*256[\s\S]*MAX_CFB_MINI_FAT_SECTORS:\s*usize\s*=\s*16[\s\S]*MAX_CFB_MINI_STREAM_BYTES:\s*usize\s*=\s*super::MAX_MAIL_HEADER_BYTES[\s\S]*MAX_CFB_TREE_NODES:\s*usize\s*=\s*MAX_CFB_DIRECTORY_ENTRIES[\s\S]*MAX_CFB_PROPERTY_SECTORS:\s*usize\s*=\s*128[\s\S]*MAX_CFB_MINI_CHAIN_SECTORS:\s*usize\s*=\s*1024[\s\S]*MAX_CFB_TOTAL_READ_BYTES:\s*usize\s*=\s*1024\s*\*\s*1024[\s\S]*MAX_MSG_PROPERTIES_STREAM_BYTES:\s*usize\s*=\s*64\s*\*\s*1024[\s\S]*MAX_MSG_PROPERTY_ENTRIES:\s*usize\s*=\s*128' `
    "Outlook CFB FAT, DIFAT, directory, mini-stream, tree, chain, and property parsing must keep explicit budgets."
Require-Pattern $nativeMailCfb 'match major_version\s*\{[\s\S]*3\s*=>\s*9[\s\S]*4\s*=>\s*12[\s\S]*read_u16\(bytes, 28\)\?\s*!=\s*0xFFFE[\s\S]*source_len\s*<\s*sector_size as u64[\s\S]*fat_sector_count\s*>\s*MAX_CFB_FAT_SECTORS[\s\S]*mini_fat_sector_count\s*>\s*MAX_CFB_MINI_FAT_SECTORS[\s\S]*fn cfb_read_fat<R:\s*Read\s*\+\s*Seek>[\s\S]*fn cfb_read_regular_chain<R:\s*Read\s*\+\s*Seek>[\s\S]*fn cfb_read_mini_chain\(' `
    "Outlook CFB v3/v4 parsing must retain byte-order validation and bounded FAT, regular-chain, and mini-chain readers."
Require-Pattern $nativeMailCfb 'struct\s+CfbSource<''a,\s*R>[\s\S]*source_len:\s*u64[\s\S]*bytes_read:\s*usize[\s\S]*fn\s+read_at\(&mut self, offset:\s*u64, length:\s*usize\)[\s\S]*preview_cancelled\(self\.cancel_cb\)[\s\S]*offset\.checked_add\(u64::try_from\(length\)\.ok\(\)\?\)[\s\S]*end\s*>\s*self\.source_len[\s\S]*self\.bytes_read\.checked_add\(length\)[\s\S]*next_total\s*>\s*MAX_CFB_TOTAL_READ_BYTES[\s\S]*seek\(SeekFrom::Start\(offset\)\)[\s\S]*read_exact_cancelable\(self\.reader, &mut bytes, self\.cancel_cb\)[\s\S]*fn\s+sector_bytes\([\s\S]*u64::from\(sector\)\.checked_add\(1\)[\s\S]*checked_mul\(header\.sector_size as u64\)[\s\S]*self\.read_at\(offset, header\.sector_size\)' `
    "Outlook CFB sectors must be sought on demand with source-length, cancellation, checked-offset, and 1 MiB cumulative-read enforcement."
...
Require-Pattern $nativeMailCfbTests 'fn\s+msg_reader_reads_regular_property_beyond_legacy_prefix\([\s\S]*property_offset\s*>\s*MAX_MAIL_HEADER_BYTES as u64[\s\S]*max_start_seek\s*>=\s*property_offset[\s\S]*bytes_read\s*<=\s*MAX_CFB_TOTAL_READ_BYTES\s*\+\s*8[\s\S]*bytes_read\s*<\s*source_len as usize[\s\S]*fn\s+cfb_source_enforces_cumulative_read_budget\([\s\S]*read_at\(0, MAX_CFB_TOTAL_READ_BYTES\s*\+\s*1\)\.is_none\(\)[\s\S]*ReaderPreviewError::LimitExceeded[\s\S]*fn\s+mail_reader_reports_length_mismatch_and_cancellation\([\s\S]*bytes\.len\(\) as u64\s*\+\s*1[\s\S]*ReaderPreviewError::LengthMismatch[\s\S]*Some\(always_cancel\)[\s\S]*ReaderPreviewError::Cancelled' `
    "Outlook CFB regressions must retain late-sector seeking, cumulative budget rejection, length mismatch, and cancellation coverage."
...
Require-Pattern $parserHostIntegration 'TransferredHandleReleaseTimeout\s*=\s*TimeSpan\.FromSeconds\(20\)' `
    "ParserHost duplicate HANDLE cleanup must keep a bounded native-cancellation release budget."
Require-Pattern $parserHostIntegration 'Duplicate_handle_request_ID_releases_every_transferred_handle[\s\S]*bool\s+previewReadyReceived\s*=\s*false[\s\S]*previewReadyReceived\s*=\s*true[\s\S]*CancellationTokenSource\(TransferredHandleReleaseTimeout\)[\s\S]*WaitForDuplicateHandleReleaseAsync\(\s*channel,\s*requestId,\s*previewReadyReceived,[\s\S]*TryOverwriteFile\(firstPath[\s\S]*releaseTimeout\.Token[\s\S]*PreviewClose\(requestId\)[\s\S]*Duplicate_SQLite_handle_request_ID_releases_both_handle_bundles' `
    "ParserHost duplicate single-HANDLE test must drain late Ready responses under an independent release timeout."
Require-Pattern $parserHostIntegration 'Duplicate_SQLite_handle_request_ID_releases_both_handle_bundles[\s\S]*bool\s+previewReadyReceived\s*=\s*false[\s\S]*previewReadyReceived\s*=\s*true[\s\S]*CancellationTokenSource\(TransferredHandleReleaseTimeout\)[\s\S]*WaitForDuplicateHandleReleaseAsync\(\s*channel,\s*requestId,\s*previewReadyReceived,[\s\S]*paths\.All\(path\s*=>\s*TryOverwriteFile[\s\S]*releaseTimeout\.Token[\s\S]*PreviewClose\(requestId\)[\s\S]*Invalid_handle_envelope_releases_the_transferred_handle' `
    "ParserHost duplicate SQLite-HANDLE test must drain late Ready responses under an independent release timeout."
Require-Pattern $parserHostIntegration 'WaitForDuplicateHandleReleaseAsync\([\s\S]*previewReadyReceived[\s\S]*CreateLinkedTokenSource\(cancellationToken\)[\s\S]*channel\.ReceiveAsync\(drainCts\.Token\)[\s\S]*WaitUntilAsync\(releaseCondition,\s*cancellationToken\)[\s\S]*drainCts\.Cancel\(\)[\s\S]*Assert\.IsType<PreviewReady>\(trailing\)' `
    "ParserHost duplicate HANDLE cleanup tests must drain one optional late Ready to avoid pipe-backpressure deadlock."
...
Require-Pattern $rasterHostIntegration 'Repeated_image_handle_previews_release_sources_without_linear_handle_growth[\s\S]*warmupCycleCount\s*=\s*64[\s\S]*baselineWindowCycleCount\s*=\s*8[\s\S]*measuredCycleCount\s*=\s*32[\s\S]*handleGrowthBudget\s*=\s*12[\s\S]*PreviewSurfaceRelease[\s\S]*host\.HandleCount[\s\S]*baselinePeakHandles\s*=\s*Math\.Max\([\s\S]*peakHandles\s*=\s*Math\.Max\([\s\S]*peakHandles\s*<=\s*baselinePeakHandles\s*\+\s*handleGrowthBudget' `
...
Require-Pattern $pdfHostIntegration 'Repeated_pdf_sessions_return_page_cache_and_projection_resources_after_idle_trim[\s\S]*Task\.Delay\(TimeSpan\.FromSeconds\(15\)[\s\S]*Assert\.False\(host\.HasExited[\s\S]*RasterHostProcessTestHelper\.AssertCleanExit' `
